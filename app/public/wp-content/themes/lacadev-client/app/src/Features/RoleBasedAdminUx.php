 ...
 *   - Custom admin color scheme per role (editor → green, author → blue)
...
class RoleBasedAdminUx
{
    public function init(): void
    {
        ...
        add_filter('get_user_option_admin_color', [$this, 'roleColorScheme'], 10, 2);
    ...
    public function roleColorScheme(string $color, string|int $userId): string
    {
        // Respect user's own choice if they've explicitly set one
        $stored = get_user_meta($userId, 'admin_color', true);
        if ($stored && $stored !== 'fresh') {
            return $stored;
        }

        $tier = $this->getTier(get_userdata($userId) ?: null);
...
#wpadminbar::after {
    ...
    private function getTier(?\WP_User $user = null): string
    {
        ...
        // Developer override: super-user logins
        $superLogins = (array) apply_filters('lacadev/super_user_logins', []);
