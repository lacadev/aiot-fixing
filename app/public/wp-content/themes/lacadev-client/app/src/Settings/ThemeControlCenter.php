...
class ThemeControlCenter
{
    ...
    public function renderPage(): void
    {
        ...
        <div class="wrap laca-cc">
            <h1 class="laca-cc__heading">
                <span>🎛</span> Theme Control Center
            ...
            <div class="laca-cc__layout">
                ...
                <nav class="laca-cc__nav" aria-label="Tab navigation">
                    <?php foreach ($tabs as $id => $tab): ?>
                        ...
                        ?>
                        <a href="<?php echo esc_url(admin_url('themes.php?page=' . self::MENU_SLUG . '&tab=' . $id)); ?>"
                    ...
                    if ($activeTabDef && is_callable($activeTabDef['render'] ?? null)):
                        ...
                        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                            ...
                            <input type="hidden" name="_laca_cc_redirect"
                                   value="<?php echo esc_url(admin_url('themes.php?page=' . self::MENU_SLUG . '&tab=' . $activeTab . '&saved=1')); ?>">
                        <?php endif; ?>
    ...
    public function handleSave(): void
    {
        ...
        $redirect = esc_url_raw($_POST['_laca_cc_redirect'] ?? admin_url('themes.php?page=' . self::MENU_SLUG));
        $tabs     = $this->getTabs();
    ...
    {
        $siteEmail   = get_option('laca_general_email',     get_option('admin_email'));
        $footerText  = get_option('laca_footer_text',       '');
        $cookieBanner= get_option('laca_cookie_banner',     '0');
        $readingMode = get_option('laca_reading_mode_enabled', '1');
        ?>
        <table class="form-table" role="presentation">
            <tr>
                <th><label for="laca_general_email">Email nhận liên hệ</label></th>
                <td>
                    <input type="email" id="laca_general_email" name="laca_general_email"
                           value="<?php echo esc_attr($siteEmail); ?>" class="regular-text">
                    <p class="description">Email nhận form liên hệ từ website.</p>
                </td>
            </tr>
            <tr>
                <th><label for="laca_footer_text">Footer text</label></th>
                <td>
                    <input type="text" id="laca_footer_text" name="laca_footer_text"
                           value="<?php echo esc_attr($footerText); ?>" class="large-text">
                </td>
            </tr>
            <tr>
                <th>Cookie Banner</th>
                <td>
                    <label>
                        <input type="checkbox" name="laca_cookie_banner" value="1"
                               <?php checked('1', $cookieBanner); ?>>
                        Hiển thị cookie consent banner
                    </label>
                </td>
            </tr>
            <tr>
    ...
    {
        update_option('laca_general_email',         sanitize_email($post['laca_general_email'] ?? ''));
        update_option('laca_footer_text',           sanitize_text_field($post['laca_footer_text'] ?? ''));
        update_option('laca_cookie_banner',         isset($post['laca_cookie_banner'])        ? '1' : '0');
